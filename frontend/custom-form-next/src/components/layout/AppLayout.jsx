  ...
  return (
    <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] flex flex-col md:flex-row">
      ...
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-50 p-2 rounded-lg bg-indigo-600 text-white"
      >
      ...
      } md:translate-x-0 transition-transform duration-300 fixed md:static md:w-64 w-64 border-r border-slate-700 bg-gradient-to-b from-slate-900 to-slate-800 flex flex-col h-screen z-40`}>
      ...
      {/* Main */}
      <main className="flex-1 flex flex-col w-full md:h-screen">
        <UseAuthLoader />
        {isDashboard && (
          <header className="h-auto md:h-20 border-b border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between px-4 md:px-8 py-4 md:py-0 bg-white/70 backdrop-blur-sm flex-shrink-0 gap-4 md:gap-0">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-[var(--text)]">Dashboard</h1>
